...
inline void dispIntroSequence(sf::RenderWindow & window, InputController & input) {
    const sf::Font & cornerstone = ::resHandlerPtr->getFont(ResHandler::Font::cornerstone);
    ...
    introText.setPosition(windowSize.x / 2.f - introText.getLocalBounds().width / 2.f,
    ...
    while (window.isOpen()) {
        ...
        case State::textIn:
            if (elapsedTime > 300000) {
                ...
                const uint8_t alpha = math::smoothstep(0, 300000, elapsedTime) * 255;
        ...
        case State::textOut:
            if (elapsedTime > 500000) {
                ...
                const uint8_t alpha = (1 - math::smoothstep(0, 400000, elapsedTime)) * 255;
                introText.setFillColor(sf::Color(255, 255, 255, alpha));
            }
            break;
