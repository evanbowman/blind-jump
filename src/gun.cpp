...
sf::Sprite* decodeDirIndex(sf::Sprite* gunSprite, char playerSpriteIndex, unsigned int timeout) {
...
gun::gun() {
	...
	const static std::string fileExts[6] = {"gunDown.png", "gunRight.png", "gunLeft.png", "gunDown2.png", "gunRight2.png", "gunLeft2.png"};
	for (int i = 0; i < 6; i++) {
...
int gun::getTimeout(sf::Time & elapsedTime) {
        if (timeout > elapsedTime.asMilliseconds()) {
...
void gun::updateShotVector(char playerSpriteIndex, effectsController& ef, float xOffset, float yOffset, userInterface& UI, InputController* pInput, SoundController& sounds, int playerState, sf::Time & elapsedTime) {
	sprIndex = playerSpriteIndex;
	if (bulletTimer == 0 && pInput->xPressed() && timeout < 1672 && playerState != 0) {
		if (sprIndex == 0 || sprIndex == 4) {  //If the gun sprite is up or down, construct a bullet with a corresponding sprite
			...
			sounds.playEffect(SoundController::Effect::gunshot);
		}
	...
	if (bulletTimer != 0 && playerState == 1) {
	        bulletTimer += elapsedTime.asMilliseconds();
		if (bulletTimer > 356) {  //If enough time has passed since the last shot, set the counter back down to shoot again
		      bulletTimer = 0;
		}
	}
