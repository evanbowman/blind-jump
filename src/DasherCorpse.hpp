...
class _DasherCorpse : public Drawable<_DasherCorpse<DrawPolicy>, DrawPolicy>,
    ...
    enum class State { falling, fallen };
    State state;

public:
    static const size_t drawOffset = 0;
    _DasherCorpse(const sf::Vector2f & _position, const sf::Texture & texture,
                  float scale)
        : Object(_position.x + 4, _position.y), frameIndex(0),
          animationTimer(0), state(State::falling) {
