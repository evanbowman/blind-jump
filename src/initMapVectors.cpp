...
void initMapVectors(tileController * pTiles) {
    ...
    do {
        ...
        (pTiles->mapArray[transporterX][transporterY] != Tile::SandAndGrass));
    ...
    for (int i = 0; i < mapSideLen; i++) {
        for (int j = 0; j < mapSideLen; j++) {
            ...
            if (tileId == Tile::Sand || tileId == Tile::SandAndGrass ||
                tileId == Tile::GrassFlowers) {
                Coordinate c1;
                c1.x = i;
                c1.y = j;
            ...
            } else if (tileId == Tile::PlateLowerEdge ||
                       tileId == Tile::GrassLowerEdge ||
