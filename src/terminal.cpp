...
Terminal::Terminal(const float _xInit, const float _yInit,
                   const sf::Texture & mainTxtr, const uint8_t tile)
    : Detail(_xInit + rng::random<4, -2>(), _yInit - rng::random<2, 1>()),
      animationTimer(0), stateTimer(0), frameIndex(0), state(State::dormant),
      screenSheet(mainTxtr), mainSprite(mainTxtr), shadow(mainTxtr) {
...
const sf::Sprite & Terminal::getSprite() const { return mainSprite; }

const sf::Sprite & Terminal::getShadow() const { return shadow; }

const sf::Sprite & Terminal::getScreen() const {
    return screenSheet[frameIndex];
}

void Terminal::setState(const Terminal::State _state) { state = _state; }
