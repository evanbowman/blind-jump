...
public:
	...
	hitBox.setPosition(position.x - 16, position.y - 16);
        ...
        case State::idle: {
            ...
            if (math::distance(playerPos.x, playerPos.y, position.x,
                               position.y) > 64) {
                ...
                if (!enemyVec.empty()) {
                    std::sort(enemyVec.begin(), enemyVec.end(),
                              [this](const auto & lhs, const auto & rhs) {
                                  const sf::Vector2f & posLhs =
                                      lhs->getPosition();
                                  const sf::Vector2f & posRhs =
                                      rhs->getPosition();
                                  const float distLhs = math::distance(
                                      this->position.x, this->position.y,
                                      posLhs.x, posLhs.y);
                                  const float distRhs = math::distance(
                                      this->position.x, this->position.y,
                                      posRhs.x, posRhs.y);
        ...
        case State::returnToPlayer: {
            ...
            const sf::Vector2f playerPos = pGame->getPlayer().getPosition();
            const float dist = math::distance(playerPos.x, playerPos.y,
            ...
            if (dist < 48) {
                ...
                approachTarget(elapsedTime, pGame, playerPos,
                               (1.f - normal) * 2.8);
            }
            runSheet.setPosition(position);
            const float xScale = cos(currentDir);
    ...
    void approachTarget(const sf::Time & elapsedTime, Game * pGame,
        ...
        if (path.empty() || recalc == 0) {
            ...
            position.y -= speed * sin(currentDir) *
                          (elapsedTime.asMicroseconds() * 0.00005f);
            if (fabs(position.x - (((path.back().x * 32) + 4 + tiles.posX))) <
                    8 &&
                fabs(position.y - (((path.back().y * 26) + 4 + tiles.posY))) <
                    8) {
                recalc--;
                path.pop_back();
                currentDir = atan2(
