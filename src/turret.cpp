...
Turret::Turret(const sf::Texture & gameObjects, float _xPos, float _yPos)
    : state(State::closed), xPos(_xPos), yPos(_yPos), frameIndex(0), timer(0),
      hp(6), killFlag(0), colorTimer(0), isColored(0), colorAmount(0) {
...
void Turret::update(const sf::Time & elapsedTime, const Player * player,
    ...
    switch (state) {
    case State::closed:
        if (std::sqrt(std::pow((xPos - player->getXpos() + 8), 2) +
                      std::pow((yPos - player->getYpos() + 16), 2)) < 174) {
            state = State::opening;
            timer = 0;
            frameIndex = 0;
        }
    ...
    case State::shoot1:
        ...
        if (timer > 200000) {
            ...
            effects.add<6>(
                ...
                yPos + 6, angleFunction(target.x + 16, target.y + 8, xPos + 18,
                                        yPos + 8));
            timer = 0;
            state = State::shoot2;
        }
    ...
    case State::shoot2:
        ...
        if (timer > 200000) {
            ...
            effects.add<6>(
                ::resHandlerPtr->getTexture(ResHandler::Texture::gameObjects),
                ::resHandlerPtr->getTexture(ResHandler::Texture::redglow), xPos,
                yPos + 8, angleFunction(target.x + 16, target.y + 8, xPos + 18,
                                        yPos + 6));
            timer = 0;
            state = State::shoot3;
        }
...
float Turret::getXpos() { return xPos; }

float Turret::getYpos() { return yPos; }
