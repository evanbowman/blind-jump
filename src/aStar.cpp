...
std::vector<aStrCoordinate> getAdjacent(aStrCoordinate & coord,
    ...
    // Look left
    if (map[coord.x - 1][coord.y] == 3 || map[coord.x - 1][coord.y] == 4 ||
        map[coord.x - 1][coord.y] == 5 || map[coord.x - 1][coord.y] == 8 ||
        map[coord.x - 1][coord.y] == 11) {
    ...
    // Inspect up
    if (map[coord.x][coord.y - 1] == 3 || map[coord.x][coord.y - 1] == 4 ||
        map[coord.x][coord.y - 1] == 5 || map[coord.x][coord.y - 1] == 8 ||
    ...
    // Diagonal directions
    if (diagonalMove) {
        if (map[coord.x + 1][coord.y + 1] == 3 ||
            map[coord.x + 1][coord.y + 1] == 4 ||
            map[coord.x + 1][coord.y + 1] == 5 ||
...
// The main function for finding a path between two points using the A*
// algorithm
std::vector<aStrCoordinate> astar_path(aStrCoordinate & origin,
    ...
    // Make the vector into a heap (priority_queue does not give access to
    // underlying vector)
