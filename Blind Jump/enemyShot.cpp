...
void Enemyshot::update(float xOffset, float yOffset) {
    xPos = xInit + xOffset + 3.4 * (65 - timeout) * (cos(direction));         // Note: timeout starts at 60, so 60 - timout grows linearly with time
    yPos = yInit + 11 + yOffset + 3.4 * (65 - timeout) * (sin(direction));
    for (int i = 0; i < 2; i++) {
