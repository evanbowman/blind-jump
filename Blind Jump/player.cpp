...
void Player::drawController(InputController* pInput) {
    //*******| MOVEMENT CODE |********
    if (slowSpeed == 1) {
        slowSpeed = 2;
    }
    ...
    if (active && !deathSeq) {
    if (!x) {      //Holding the x key strafes the player and changes the animation, so account for that condition
        if (up) {
